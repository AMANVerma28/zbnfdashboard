...
def standard_deviation(data,k):
  differences=[]
  c=0
  for i in range(1,row+1):
    if data[i] is not None:
      #print("da",data[i],"av",average[k])
      differences.append(data[i]-average[k])
      c=c+1
...
# Reading wells data
data = []
#maReader = csv.reader(open('wells2.csv', 'rb'), delimiter=',', quotechar='"')
with open('../static/json/analyticsyield.json') as data_jsonfile:
    yielddata = json.load(data_jsonfile)
with open('../static/json/analyticswell.json') as data_jsonfile:
    welldata = json.load(data_jsonfile)
rindex = len(welldata)
iindex = int(len(yielddata)/rindex)

for i in range(0,rindex+1):
    data.append([])

k = 0
for i in range(1,rindex+1):
    data[i].append(i)
    for j in range(1,iindex+1):
        #print ("Rindex",rindex,"Index",iindex,k)
        data[i].append(yielddata[k]['Yield'])
        #print(data[i])
        k = k+1 
        #print (k)
...
for data_record in data[1: ]:
  #print(data_record)
  sta_dev.append(standard_deviation(data_record,k))
  k = k+1
...
# Reading lon lat
latlon=[]
maReader = csv.reader(open('latlon.csv', 'rb'), delimiter=',',quotechar='"')
for rindex, row in enumerate(maReader):
  for iindex, item in enumerate(row):
    try:
...
'''
with open('latlon.csv') as fpi, open('out.csv', 'w') as fpo:
